...
class TestSample:
    ...
    def test_blank_init(self):
        """Tests initialization with no parameters"""
    ...
    def test_keys_on_blank_init(self):
        """Tests that recommended keys exist on blank initialization"""
        sample = Sample()
    ...
    def test_promotion_of_read_structure(self):
        """Tests that a read_structure key is promoted to ``ReadStructure``"""
        sample = Sample({
    ...
    def test_eq(self):
        """Tests equality  based only on ``sample_id`` and ``library_id``"""
    ...
    def test_str(self):
        """Tests ``sample.__str__()``"""
        eq_(str(Sample()), 'None')
        eq_(str(Sample({'sample_id': 245})), '245')

    def test_repr(self):
        """Tests ``Sample.__repr__()`` after initialization"""
        eq_(Sample().__repr__(),
